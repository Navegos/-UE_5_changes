...
namespace UnrealBuildTool
{
	class LinuxToolChain : ClangToolChain
	{
			...
			: this(UnrealTargetPlatform.Linux, InArchitecture, InSDK, InOptions, InLogger)
		{
			...
			if (CompilerVersionLessThan(15, 0, 0) || CompilerVersionGreaterOrEqual(19, 0, 0))
			{
				throw new BuildException(
					String.Format("This version of the Unreal Engine can only be compiled with clang 15.x or 18.x. clang {0} may not build it - please use a different version.",
						Info.ClangVersion)
					);
			}
		}
		...
		protected override void GetCompileArguments_Optimizations(CppCompileEnvironment CompileEnvironment, List<string> Arguments)
		{
			...
			// DeveVitorF already defined in ClangToolChain
