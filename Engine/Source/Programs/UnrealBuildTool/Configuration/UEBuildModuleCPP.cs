...
namespace UnrealBuildTool
{
	...
	class UEBuildModuleCPP : UEBuildModule
	{
		...
		internal static bool IsCompatibleForSharedPCH(CppCompileEnvironment ModuleCompileEnvironment, CppCompileEnvironment CompileEnvironment)
		{
			...
			if (ModuleCompileEnvironment.DefaultCpuArchX64 != CompileEnvironment.DefaultCpuArchX64)
			{
				return false;
			}

			if (ModuleCompileEnvironment.MinCpuArchX64 != CompileEnvironment.MinCpuArchX64)
			{
				return false;
			}
		...
		private static string GetSuffixForSharedPCH(CppCompileEnvironment CompileEnvironment, CppCompileEnvironment BaseCompileEnvironment)
		{
			...
			if (CompileEnvironment.DefaultCpuArchX64 != BaseCompileEnvironment.DefaultCpuArchX64)
			{
				Variant += String.Format(".{0}", CompileEnvironment.DefaultCpuArchX64);
			}

			if (CompileEnvironment.MinCpuArchX64 != BaseCompileEnvironment.MinCpuArchX64)
			{
				Variant += String.Format(".{0}", CompileEnvironment.MinCpuArchX64);
			}
		...
		private static void CopySettingsForSharedPCH(CppCompileEnvironment ModuleCompileEnvironment, CppCompileEnvironment CompileEnvironment)
		{
			...
			CompileEnvironment.DefaultCpuArchX64 = ModuleCompileEnvironment.DefaultCpuArchX64;
			CompileEnvironment.MinCpuArchX64 = ModuleCompileEnvironment.MinCpuArchX64;
		...
		public CppCompileEnvironment CreateModuleCompileEnvironment(ReadOnlyTargetRules Target, CppCompileEnvironment BaseCompileEnvironment, ILogger Logger)
		{
			...
			// If the module overrides the x64 minimum arch, override it on the compile environment
			if (Rules.DefaultCpuArchX64 != null)
			{
				Result.DefaultCpuArchX64 = (DefaultCpuArchitectureX64)Rules.DefaultCpuArchX64;
			}
			...
			if (Rules.DefaultCpuArchX64 != Target.DefaultCpuArchX64)
			{
				if ((Rules.MinCpuArchX64 >= MinimumCpuArchitectureX64.AVX2) && (Target.DefaultCpuArchX64 < DefaultCpuArchitectureX64.Haswell))
				{
					Result.DefaultCpuArchX64 = DefaultCpuArchitectureX64.Haswell;
				}
				else
				{
					if (Rules.DefaultCpuArchX64 != null)
			...
			{
				if ((Rules.MinCpuArchX64 >= MinimumCpuArchitectureX64.AVX2) && (Rules.DefaultCpuArchX64 < DefaultCpuArchitectureX64.Haswell))
				{
					Result.DefaultCpuArchX64 = DefaultCpuArchitectureX64.Haswell;
				}
				else
			...
			if (Result.DefaultCpuArchX64 >= DefaultCpuArchitectureX64.Haswell)
			{
				...
				else if (Result.MinCpuArchX64 != Rules.MinCpuArchX64)
				{
					if (Rules.MinCpuArchX64 != null)
					{
						Result.MinCpuArchX64 = (MinimumCpuArchitectureX64)Rules.MinCpuArchX64;
