...
class CanonicalReader : public dna::FakeReader {
    public:
        ...
        av::ConstArrayView<std::uint16_t> getNeuralNetworkIndicesForLOD(std::uint16_t lod) const override {
            return av::ConstArrayView<std::uint16_t>{unoptimized::neuralNetworkIndicesPerLOD[lod]};
        }

        av::ConstArrayView<std::uint16_t> getNeuralNetworkInputIndices(std::uint16_t neuralNetIndex) const override {
            return av::ConstArrayView<std::uint16_t>{unoptimized::mlbNetInputIndices[neuralNetIndex]};
        }

        av::ConstArrayView<std::uint16_t> getNeuralNetworkOutputIndices(std::uint16_t neuralNetIndex) const override {
            return av::ConstArrayView<std::uint16_t>{unoptimized::mlbNetOutputIndices[neuralNetIndex]};
        }
        ...
        av::ConstArrayView<float> getNeuralNetworkLayerActivationFunctionParameters(std::uint16_t neuralNetIndex,
                                                                                std::uint16_t layerIndex) const override {
            return av::ConstArrayView<float>{unoptimized::mlbNetActivationFunctionParameters[neuralNetIndex][layerIndex]};
        }

        av::ConstArrayView<float> getNeuralNetworkLayerBiases(std::uint16_t neuralNetIndex, std::uint16_t layerIndex) const override {
            return av::ConstArrayView<float>{unoptimized::mlbNetBiases[neuralNetIndex][layerIndex]};
        }

        av::ConstArrayView<float> getNeuralNetworkLayerWeights(std::uint16_t neuralNetIndex,
                                                           std::uint16_t layerIndex) const override {
            return av::ConstArrayView<float>{unoptimized::mlbNetWeights[neuralNetIndex][layerIndex]};
        }
