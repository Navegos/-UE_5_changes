...
namespace
{
	...
	NRDCVar<int32> RelaxHistoryDiffuseMaxFastAccumulatedFrameNum(TEXT("r.NRD.Relax.History.DiffuseFastMaxAccumulatedFrameNum"), 8, TEXT("Amount of frames in history for fast responsive signal temporal accumulation "), 0, RELAX_MAX_HISTORY_FRAME_NUM);
	...
	NRDCVar<float> RelaxReprojectionDisocclusionThreshold(TEXT("r.NRD.Relax.Reprojection.DisocclusionThreshold"), 0.05f, TEXT("Percentage of the depth value for disocclusion detection / geometry tests"), 0.001f, 1.0f);
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER(FMatrix44f, gWorldToClip)
		SHADER_PARAMETER(FMatrix44f, gWorldToView)
		SHADER_PARAMETER(FMatrix44f, gViewToClip)
		SHADER_PARAMETER(FVector4f, gRotator) 
		SHADER_PARAMETER(FVector4f, gFrustumRight) 
		SHADER_PARAMETER(FVector4f, gFrustumUp) 
		SHADER_PARAMETER(FVector4f, gFrustumForward) 
		SHADER_PARAMETER(FIntPoint, gRectOrigin) 
		SHADER_PARAMETER(FVector2f, gRectOffset)
		SHADER_PARAMETER(FIntPoint, gResolution)
		SHADER_PARAMETER(FVector2f, gInvViewSize) 
		SHADER_PARAMETER(FVector2f, gInvRectSize) 
		SHADER_PARAMETER(FVector2f, gResolutionScale) 
...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		SHADER_PARAMETER_STRUCT_INCLUDE(FNRDCommonSamplerParameters, CommonSamplers)
		SHADER_PARAMETER(FMatrix44f,   gPrevWorldToClip)
		SHADER_PARAMETER(FVector4f,  gFrustumRight)
		SHADER_PARAMETER(FVector4f,  gFrustumUp)
		SHADER_PARAMETER(FVector4f,  gFrustumForward)
		SHADER_PARAMETER(FVector4f,  gPrevFrustumRight)
		SHADER_PARAMETER(FVector4f,  gPrevFrustumUp)
		SHADER_PARAMETER(FVector4f,  gPrevFrustumForward)
		SHADER_PARAMETER(FVector3f,   gPrevCameraPosition)
		SHADER_PARAMETER(float,     gJitterDelta)
		SHADER_PARAMETER(FVector2f, gMotionVectorScale)
		SHADER_PARAMETER(FIntPoint, gResolution)
		SHADER_PARAMETER(FVector2f, gInvViewSize)
		...
		SHADER_PARAMETER(FIntPoint, gRectOrigin) 
		SHADER_PARAMETER(FVector2f, gInvRectSize)
		SHADER_PARAMETER(FVector2f, gRectSizePrev)
	...
	)
{
	...
	FMatrix44f WorldToViewMatrix = FMatrix44f(View.ViewMatrices.GetTranslatedViewMatrix());
	FMatrix44f WorldToViewMatrixPrev = FMatrix44f(View.PrevViewInfo.ViewMatrices.GetTranslatedViewMatrix());
	FMatrix44f WorldToClipMatrixPrev = FMatrix44f(View.PrevViewInfo.ViewMatrices.GetTranslatedViewProjectionMatrix().GetTransposed());
	FMatrix44f ViewToClipMatrix = FMatrix44f(View.ViewMatrices.GetProjectionMatrix());
	FMatrix44f ViewToClipMatrixPrev = FMatrix44f(View.PrevViewInfo.ViewMatrices.GetProjectionMatrix());
	...
	FVector3f FrustumRight = WorldToViewMatrix.GetColumn(0) * TanHalfFov;
	FVector3f FrustumUp = WorldToViewMatrix.GetColumn(1) * TanHalfFov * Aspect;
	FVector3f FrustumForward = WorldToViewMatrix.GetColumn(2);
	...
	FVector3f PrevFrustumRight = WorldToViewMatrixPrev.GetColumn(0) * TanHalfFov;
	FVector3f PrevFrustumUp = WorldToViewMatrixPrev.GetColumn(1) * PrevTanHalfFov * PrevAspect;
	FVector3f PrevFrustumForward = WorldToViewMatrixPrev.GetColumn(2);

	// subrect parameters
	// EHartNV TODO - replace with proper subrect support
	const FIntPoint ViewRectOrigin = FIntPoint(0, 0);
	const FVector2f ViewRectSize = View.ViewRect.Size();
	const FVector2f InvViewRectSize = FVector2f(1.0f / ViewRectSize.X, 1.0f / ViewRectSize.Y);
	const FVector2f ViewRectSizePrev = ViewRectSize;
	...
	FRDGTextureRef PrepassScaledViewZ = CreateIntermediateTexture(PF_R16F, TEXT("NRD.Relax.Prepass.ScaledViewZ"));
	{
		...
		PassParameters->gWorldToClip = FMatrix44f(View.ViewMatrices.GetTranslatedViewProjectionMatrix().GetTransposed());
		PassParameters->gWorldToView = FMatrix44f(View.ViewMatrices.GetTranslatedViewMatrix().GetTransposed());
		PassParameters->gViewToClip = FMatrix44f(View.ViewMatrices.GetProjectionMatrix().GetTransposed());

		// EHartNV TODO - should be a white-noise rotation matrix cos, sin, -sin, cos
		//PassParameters->gRotator = FVector4f(1.0f, 0.0f, 0.0f, 1.0f);
		float ModIndex = History->FrameIndex % 63;
		float Phi = (ModIndex * 1.61803398875f) * 2.0f * 3.1415927;
		float CosPhi = FMath::Cos(Phi);
		float SinPhi = FMath::Sin(Phi);
		PassParameters->gRotator = FVector4f(CosPhi, SinPhi, -SinPhi, CosPhi);
		...
		PassParameters->gRectOffset = FVector2f(0.0f,0.0f); // this is the offset in UV space
		...
		PassParameters->gInvRectSize = InvViewRectSize;
		PassParameters->gResolutionScale = FVector2f(1.0f, 1.0f); 
	...
	FRDGTextureRef ReprojectSpecularReprojectionConfidence = CreateIntermediateTexture(PF_R16F, TEXT("NRD.Relax.Reproject.SpecularReprojectionConfidence"));
	{
		...
		PassParameters->gPrevCameraPosition = FVector3f(-View.ViewMatrices.GetViewOrigin() + View.PrevViewInfo.ViewMatrices.GetViewOrigin());
		PassParameters->gJitterDelta = JitterDelta;
		PassParameters->gMotionVectorScale = FVector2f(1.0f, 1.0f);
		PassParameters->gResolution = DenoiseBufferSize;
		PassParameters->gInvViewSize = FVector2f(1.0f, 1.0f) / FVector2f(DenoiseBufferSize);
	...
	FRDGTextureRef DisocclusionFixDiffuseIlluminationResponsive = CreateIntermediateTexture(PF_FloatRGBA, TEXT("NRD.Relax.DisocclusionFix.DiffuseIlluminationResponsive"));
	{
		...
		PassParameters->gFrustumForward = FrustumForward;
		PassParameters->gResolution = DenoiseBufferSize;
		PassParameters->gInvRectSize = FVector2f(1.0f, 1.0f) / FVector2f(DenoiseBufferSize);
	...
	FRDGTextureRef SpatialVarianceEstimationNormalRoughness = CreateIntermediateTexture(NRDGetNormalRoughnessFormat(), TEXT("NRD.Relax.SpatialVarianceEstimation.NormalRoughness"));
	{
		...
		PassParameters->gNormalRoughness = GraphBuilder.CreateSRV(FRDGTextureSRVDesc::Create(Inputs.NormalAndRoughness));
		PassParameters->gViewZ = GraphBuilder.CreateSRV(FRDGTextureSRVDesc::Create(PrepassScaledViewZ));
	...
	FRDGTextureRef FinalAtrousOutputSpecular = nullptr;
	{
		...
		check(2 <= RelaxSettings.atrousIterationNum && RelaxSettings.atrousIterationNum <= 8);
	...
	if(0 != SplitScreenPercentage)
	{
		...
		PassParameters->gInvRectSize = FVector2f( 1.0f / DenoiseBufferSize.X, 1.0f / DenoiseBufferSize.Y) ;
