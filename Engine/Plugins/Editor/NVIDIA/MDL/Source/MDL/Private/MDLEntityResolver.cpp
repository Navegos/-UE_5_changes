...
IReader *FMDLImportResult::create_reader() const
{
	...
	{
		FileHandle = FileReader->OpenRead(*FString(FileName.c_str()));
	}
	else // assume local
	{
		IPlatformFile& PlatformFile = FPlatformFileManager::Get().GetPlatformFile();
		FileHandle = PlatformFile.OpenRead(*FString(FileName.c_str()));
	}
...
{
	ResourceEntries.Add(FResourceEntry(url, filename, 0, 0));
}
...
{
    return ResourceEntries.Num();
}

// Get the i'th file name of the ordered set.
char const *FMDLResourceSet::get_filename(mi::Size i) const
{
    if (i < ResourceEntries.Num())
	{
        return ResourceEntries[i].FileName.c_str();
	}
    return nullptr;
}

// Get the i'th MDL url of the ordered set.
char const *FMDLResourceSet::get_mdl_file_path(mi::Size i) const
{
    if (i < ResourceEntries.Num())
	{
        return ResourceEntries[i].Url.c_str();
	}
    return nullptr;
}

// If the ordered set represents an UDIM mapping, returns it, otherwise nullptr.
bool FMDLResourceSet::get_uvtile_uv(mi::Size i, mi::Sint32 &u, mi::Sint32 &v) const
{
    return false;
}

// Opens a reader for the i'th entry.
IReader *FMDLResourceSet::create_reader(mi::Size i) const
{
    return nullptr;
}
...
mi::base::Uuid FMDLResourceSet::get_resource_hash(mi::Size i) const
{
	return mi::base::Uuid();
}
	...
	IMdl_execution_context* context)
{
	...
	FString UnmangledFilePath = UnmangleMdlPath(file_path, false);
	...
	if (owner_name)
	{
		UnmangledOwnerName = UnmangleMdlPath(FString(owner_name));
	}
	...
	mi::Sint32 pos_column)
{
    ...
    std::string module_file_system_path_str(
        owner_file_path == nullptr ? "" : owner_file_path);
    ...
    split_module_file_system_path(
        ...
        std::string(owner_name != nullptr ? owner_name : ""),
        nesting_level,
	...
	IMdl_execution_context* context)
{
	...
	MdlUrl = UnmangleMdlPath(MdlUrl, false);
	FString UnmangledOwnerName;
	if (owner_name)
	{
		UnmangledOwnerName = UnmangleMdlPath(FString(owner_name));
    ...
    char const *fname, char const *fmask) const
{
	...
	char const *p_path = strstr(joined_file_name.c_str(), ":");
    ...
    if (p_path != nullptr && p_archive == nullptr && p_mdle == nullptr)
	{
		if (MdlFileReader.IsValid())
		{
			return MdlFileReader->FileExists(fname, fmask ? fmask : "");
		}
		else // assume local file
		{
			return FPaths::FileExists(joined_file_name.c_str());
		}
    }
...
void FMDLEntityResolver::AddResourcePath(const FString& Path)
{
}

void FMDLEntityResolver::RemoveResourcePath(const FString& Path)
{
}
