...
bool FMDLMaterialImporter::ImportMaterial(UMaterial* Material,
	const mi::base::Handle<const mi::neuraylib::IMaterial_definition>& InMaterialDefinition, const mi::base::Handle<const mi::neuraylib::ICompiled_material>& InCompiledMaterial, UMaterialExpressionClearCoatNormalCustomOutput*& OutClearCoatNormalCustomOutput, FLoadTextureCallback InCallback)
{
...
# endif
	...
	CurrentNormalExpression = NewMaterialExpressionFunctionCall(CurrentUE4Material, ::LoadFunction(FMDLImporterUtility::GetProjectFunctionPath(), TEXT("mdl_state_geometry_normal")), {});
	...
	else
	{
		...
		CurrentNormalExpression = NewMaterialExpressionFunctionCall(CurrentUE4Material, ::LoadFunction(FMDLImporterUtility::GetProjectFunctionPath(), TEXT("mdl_state_normal")), {});
	}
	...
	if (!CutoutOpacity.ExpressionData.IsDefault && TranslucentOpacity.ExpressionData.IsDefault)
	{
		if (CurrentUE4Material->BlendMode != BLEND_Translucent) 
		{
			...
			// NOTE: UE4.27 has masked shadow issue when DitherOpacityMask is enabled
			// It might be fixed at the next release. (UE5 works fine)
			...
			Cast<UMaterialExpressionConstant3Vector>(IORExpression[0].ExpressionData.Expression)->Constant == FLinearColor(1.0f, 1.0f, 1.0f, 1.0f)))
	{
		CurrentUE4Material->MaterialAttributes.Connect(0,
			NewMaterialExpressionFunctionCall(CurrentUE4Material,
				::LoadFunction(FMDLImporterUtility::GetProjectFunctionPath(), TEXT("mdlimporter_set_refraction")),
				{
					CurrentUE4Material->MaterialAttributes.Expression,
			...
			if (Semantic == mi::neuraylib::IFunction_definition::DS_INTRINSIC_DF_WEIGHTED_LAYER)
			{
				...
				if (Semantic == mi::neuraylib::IFunction_definition::DS_INTRINSIC_DF_NORMALIZED_MIX)
				{
					...
					mi::base::Handle<const mi::neuraylib::IExpression_direct_call> TmpCall(
						LookupCall(TEXT("components.1"), CM, ParentCall.get()));
					OutMaterial[TEXT("metallic")].BakePath = PathPrefix + TEXT("components.1.weight");
					OutMaterial[TEXT("metallic")].Expr = GetArgument(CM, TmpCall.get(), TEXT("weight"));

					// And other metallic parameters
					TmpCall = LookupCall(TEXT("components.1.component"), CM, ParentCall.get());

					OutMaterial[TEXT("roughness")].BakePath = PathPrefix + TEXT("components.1.component.roughness_u");
					OutMaterial[TEXT("roughness")].Expr = GetArgument(CM, TmpCall.get(), TEXT("roughness_u"));

					// Base_color can be taken from any of the leaf-bsdfs. It is supposed to
					// be the same.
					OutMaterial[TEXT("base_color")].BakePath = PathPrefix + TEXT("components.1.component.tint");
					OutMaterial[TEXT("base_color")].Expr = GetArgument(CM, TmpCall.get(), TEXT("tint"));

					// Chain further
					ParentCall = LookupCall(
						TEXT("components.0.component"), CM, ParentCall.get());
					Semantic = GetCallSemantic(MDLModule->Transaction.get(), ParentCall.get());
					PathPrefix += TEXT("components.0.component.");
				}
				if (Semantic == mi::neuraylib::IFunction_definition::DS_INTRINSIC_DF_CUSTOM_CURVE_LAYER)
				{
...
void FMDLMaterialImporter::SetPropertiesFromAnnotation(const mi::base::Handle<const mi::neuraylib::IMaterial_definition>& InMaterialDefinition)
{
	const mi::base::Handle<const mi::neuraylib::IAnnotation_block> AnnotationBlock(InMaterialDefinition->get_annotations());
	if (AnnotationBlock)
	{
		FString EncodedModuleName(InMaterialDefinition->get_mdl_module_name());
		const mi::IString* DecodedModuleName = MDLModule->MDLFactory->decode_name(TCHAR_TO_UTF8(*EncodedModuleName));
		FString ModuleName = UnmangleMdlPath(DecodedModuleName->get_c_str());
...
bool FMDLMaterialImporter::IsDistillOff(const mi::base::Handle<const mi::neuraylib::IMaterial_definition>& InMaterialDefinition)
{
	const mi::base::Handle<const mi::neuraylib::IAnnotation_block> AnnotationBlock(InMaterialDefinition->get_annotations());
	if (AnnotationBlock)
	{
...
UTexture* FMDLMaterialImporter::LoadTexture(const FString& RelativePath, const mi::base::Handle<const mi::neuraylib::ITexture>& MDLTexture, TextureCompressionSettings InCompression)
{
	UTexture* Texture = nullptr;

	if (MDLTexture)
	{
		FString Filename(MDLTexture->get_image());
		...
		if (Filename.StartsWith(MDLModule->MDLUserPath))
		{
			...
			AssetDir = FMDLImporterUtility::GetProjectMaterialPath() / PackageName;
		}
		else
		{
		...
		{
			LoadTextureCallback(Texture, Filename, MDLTexture->get_effective_gamma(), InCompression);
			if (Texture)
			{
				...
				if (MDLTexture->get_effective_gamma() > 0.0f)
				{
					Texture->SRGB = (MDLTexture->get_effective_gamma() != 1.0f);
				}
			}
		}
		...
		if (Texture == nullptr)
		{
			...
			Texture = LoadTexture(Filename, AssetDir, AssetName, MDLTexture->get_effective_gamma() != 1.0f, InCompression);
		}
	}
...
TArray<FMaterialExpressionConnection> FMDLMaterialImporter::CreateExpression(const mi::base::Handle<const mi::neuraylib::ICompiled_material>& CompiledMaterial, const mi::base::Handle<const mi::neuraylib::IExpression>& MDLExpression, const FString& CallPath)
{
	...
	switch (kind)
	{
		...
		case mi::neuraylib::IExpression::EK_DIRECT_CALL:
			...
			if (InvalidFunctionCalls.Num() == 0 && Outputs.Num() == 0)
			{
...
TArray<FMaterialExpressionConnection> FMDLMaterialImporter::CreateExpressionConstant(const mi::base::Handle<const mi::neuraylib::IValue>& MDLConstant)
{
	...
	switch (Kind)
	{
		...
		case mi::neuraylib::IValue::VK_TEXTURE:
		{
			...
			const mi::base::Handle<const mi::neuraylib::ITexture> Texture(MDLModule->Transaction->access<mi::neuraylib::ITexture>(MDLConstant.get_interface<const mi::neuraylib::IValue_texture>()->get_value()));

			return { NewMaterialExpressionTextureObject(CurrentUE4Material, LoadTexture(Value->get_file_path(), Texture)) };
		}
...
static FString MangleAssetPath(const FString& UnMangledPath)
{
	FString MangledPath;
	...
	for (auto FolderName : OutFolderNames)
	{
		...
		if (bMangledChar)
		{
			MangledFolderName += TEXT("_MANGLED");
		}
...
TArray<FMaterialExpressionConnection> FMDLMaterialImporter::MakeFunctionCall(const FString& CallPath, const mi::base::Handle<const mi::neuraylib::IFunction_definition>& FunctionDefinition, int32 ArraySize,
	const FString& AssetNamePostfix, TArray<FMaterialExpressionConnection>& Inputs)
{
	check(FunctionDefinition != nullptr);

	FString AssetPath = FMDLImporterUtility::GetProjectFunctionPath();
	FString EncodedFunctionName(FunctionDefinition->get_mdl_name());
	const mi::IString* DecodedFunctionName = MDLModule->MDLFactory->decode_name(TCHAR_TO_UTF8(*EncodedFunctionName));
	FString FunctionName(DecodedFunctionName->get_c_str());
	FString EncodedModuleName(FunctionDefinition->get_module());
	const mi::IString* DecodedModuleName = MDLModule->MDLFactory->decode_name(TCHAR_TO_UTF8(*EncodedModuleName));
	FString FunctionModuleName(DecodedModuleName->get_c_str());
	...
	{
		if (Inputs[NormalmapIndex].ExpressionData.Expression && Inputs[NormalmapIndex].ExpressionData.Expression->IsA<UMaterialExpressionTextureBase>())
		{
			auto TextureObject = Cast<UMaterialExpressionTextureBase>(Inputs[0].ExpressionData.Expression);
	...
	TArray<FMaterialExpressionConnection> Outputs;
	UMaterialFunction* Function = ::LoadFunction(AssetPath, AssetName, ArraySize);
	...
	if (!Function)
	{
		...
		Function = ::LoadFunction(AssetPath, FunctionName, ArraySize);
	}
	...
	{
		InvalidFunctionCalls.Add(AssetName);
	}
...
TArray<FMaterialExpressionConnection> FMDLMaterialImporter::CreateExpressionFunctionCall(const mi::base::Handle<const mi::neuraylib::ICompiled_material>& CompiledMaterial, const mi::base::Handle<const mi::neuraylib::IExpression_direct_call>& MDLFunctionCall, const FString& CallPath)
{
	...
	const mi::IString* DecodedFunctionName = MDLModule->MDLFactory->decode_name(TCHAR_TO_UTF8(*EncodedFunctionName));
	FString FunctionName(DecodedFunctionName->get_c_str());
	FString EncodedModuleName(FunctionDefinition->get_mdl_module_name());
	const mi::IString* DecodedModuleName = MDLModule->MDLFactory->decode_name(TCHAR_TO_UTF8(*EncodedModuleName));
	FString FunctionModuleName(DecodedModuleName->get_c_str());
	...
	for (mi::Size i = 0; i < Arguments->get_size(); i++)
	{
		...
		if (ParameterKind != mi::neuraylib::IType::TK_STRING)
		{
			...
			{
				check(0 == ArraySize);
				ArraySize = CalcArraySize(Argument);
				check(0 < ArraySize);
			}
		...
		else if (Semantic == mi::neuraylib::IFunction_definition::DS_INTRINSIC_SCENE_DATA_LOOKUP_FLOAT3
		...
		{
			if (Argument->get_kind() == mi::neuraylib::IExpression::EK_CONSTANT)
			{
				...
				if (Constant->get_kind() == mi::neuraylib::IValue::VK_STRING)
				{
					...
					if (String.Equals("displayColor"))
					{
						Inputs.Append({ NewMaterialExpressionConstant(CurrentUE4Material, 0.0f) });
					}
					else if (String.Equals("displayOpacity"))
					{
						Inputs.Append({ NewMaterialExpressionConstant(CurrentUE4Material, 1.0f) });
					}
		...
		case mi::neuraylib::IFunction_definition::DS_INTRINSIC_TEX_LOOKUP_COLOR:
			{
				...
				if (ParameterKind == mi::neuraylib::IType::TK_TEXTURE)
				{
					...
					case mi::neuraylib::IType_texture::Shape::TS_2D:
						if (Inputs[0].ExpressionData.Expression && Inputs[0].ExpressionData.Expression->IsA<UMaterialExpressionTextureBase>())
						{
							auto TextureObject = Cast<UMaterialExpressionTextureBase>(Inputs[0].ExpressionData.Expression);
							if (TextureObject->Texture && TextureObject->Texture->IsNormalMap())
							{
	...
	switch (Semantic)
	{
		...
		case mi::neuraylib::IFunction_definition::DS_INTRINSIC_SCENE_DATA_LOOKUP_COLOR:
			return MakeFunctionCall(CallPath, FunctionDefinition, ArraySize, AssetNamePostfix, Inputs);
			...
			for (int32 i = 1; i <= NumOutputs; i++)
			{
				...
				else if (IsMaterialAttribute(Inputs[i]))
				{
					...
					Outputs.Add(NewMaterialExpressionFunctionCall(CurrentUE4Material,
						::LoadFunction(FMDLImporterUtility::GetProjectFunctionPath(), TEXT("mdlimporter_select_bsdf")), { Inputs[0], Inputs[i], Inputs[i + NumOutputs], CurrentNormalExpression }));
				}
				else
				{
...
#pragma region DAG Backend Intrinsics
		...
		{
			FString OmniSharedModule = MangleMdlPath("::OmniSurface::OmniShared");
			FString OmniSharedScatteringColors = "scattering_colors";
			FString OmniSharedScatteringColors_TransmissionColor = "scattering_transmission_color";		
			FString OmniReallusionEyeColorMakeOutput = "BaseColorMakeOutput";
			FString OmniReallusionEyeColorMakeOutput_BaseColor = "base_color";
			FString OmniMinecraftTextureLookupData = "texture_lookup_data";
			FString OmniMinecraftTextureLookupData_Tint = "tint";
			FString OmniMinecraftTextureLookupData_Mono = "mono";
			FString OmniMinecraftTextureLookupData_Normal = "normal";
			FString OmniGltfTextureLookupData = "gltf_texture_lookup_value";
			FString OmniGltfTextureLookupData_Valid = "valid";
			FString OmniGltfTextureLookupData_Value = "value";
			FString OmniGltfNormalTextureLookupData = "gltf_normal_texture_lookup_value";
			FString OmniGltfNormalTextureLookupData_Valid = "valid";
			FString OmniGltfNormalTextureLookupData_Value = "value";
			...
			else if (FunctionName == OmniSharedModule + "::" + OmniSharedScatteringColors + "." + OmniSharedScatteringColors_TransmissionColor + "(" + OmniSharedModule + "::" + OmniSharedScatteringColors + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 2));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[0] });
			}
			else if (FunctionName == FunctionModuleName + "::" + OmniReallusionEyeColorMakeOutput + "." + OmniReallusionEyeColorMakeOutput_BaseColor + "(" + FunctionModuleName + "::" + OmniReallusionEyeColorMakeOutput + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 1));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[0] });
			}
			else if (FunctionName == FunctionModuleName + "::" + OmniMinecraftTextureLookupData + "." + OmniMinecraftTextureLookupData_Tint + "(" + FunctionModuleName + "::" + OmniMinecraftTextureLookupData + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 3));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[0] });
			}
			else if (FunctionName == FunctionModuleName + "::" + OmniMinecraftTextureLookupData + "." + OmniMinecraftTextureLookupData_Mono + "(" + FunctionModuleName + "::" + OmniMinecraftTextureLookupData + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 3));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[1] });
			}
			else if (FunctionName == FunctionModuleName + "::" + OmniMinecraftTextureLookupData + "." + OmniMinecraftTextureLookupData_Normal + "(" + FunctionModuleName + "::" + OmniMinecraftTextureLookupData + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 3));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[2] });
			}
			else if (FunctionName == FunctionModuleName + "::" + OmniGltfTextureLookupData + "." + OmniGltfTextureLookupData_Valid + "(" + FunctionModuleName + "::" + OmniGltfTextureLookupData + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 2));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[0] });
			}
			else if (FunctionName == FunctionModuleName + "::" + OmniGltfTextureLookupData + "." + OmniGltfTextureLookupData_Value + "(" + FunctionModuleName + "::" + OmniGltfTextureLookupData + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 2));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[1] });
			}
			else if (FunctionName == FunctionModuleName + "::" + OmniGltfNormalTextureLookupData + "." + OmniGltfNormalTextureLookupData_Valid + "(" + FunctionModuleName + "::" + OmniGltfNormalTextureLookupData + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 2));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[0] });
			}
			else if (FunctionName == FunctionModuleName + "::" + OmniGltfNormalTextureLookupData + "." + OmniGltfNormalTextureLookupData_Value + "(" + FunctionModuleName + "::" + OmniGltfNormalTextureLookupData + ")")
			{
				check((Inputs.Num() == 0) || (Inputs.Num() == 2));
				return (Inputs.Num() == 0) ? Inputs : TArray<FMaterialExpressionConnection>({ Inputs[1] });
			}
			else
			{
				...
				unimplemented_control_path(TEXT("Unhandled intrinsic 'DS_INTRINSIC_DAG_FIELD_ACCESS'"));
			}
		}
					...
					if (Index < 3)
					{
						if(Inputs[0].ExpressionData.Expression->IsA<UMaterialExpressionMaterialFunctionCall>())
						{
							...
							if (FunctionCall->MaterialFunction)
							{
								...
								{
									UMaterialFunction* ReplaceFunction = LoadFunction(FMDLImporterUtility::GetProjectFunctionPath(), MaterialFunctionName + TEXT("_") + ((Index == 0) ? "x" : ((Index == 1) ? "y" : "z")));
									if (MaterialFunctionName == TEXT("mdl_state_geometry_normal"))
									{
		...
		case mi::neuraylib::IValue::VK_TEXTURE:
			{
				...
				Parameter.Add(NewMaterialExpressionTextureObjectParameter(CurrentUE4Material, Name,
					LoadTexture(TextureValue->get_file_path(), mi::base::make_handle(MDLModule->Transaction->access<mi::neuraylib::ITexture>(Value.get_interface<const mi::neuraylib::IValue_texture>()->get_value())), InCompression)));
			}
			break;
		case mi::neuraylib::IValue::VK_LIGHT_PROFILE:
			{
				...
				Parameter.Add(NewMaterialExpressionTextureObjectParameter(CurrentUE4Material, Name,
					LoadTexture(LightValue->get_file_path(), mi::base::make_handle(MDLModule->Transaction->access<mi::neuraylib::ITexture>(Value.get_interface<const mi::neuraylib::IValue_light_profile>()->get_value())), InCompression)));
			}
			break;
...
void FMDLMaterialImporter::ImportParameters(const mi::base::Handle<const mi::neuraylib::IMaterial_definition>& MaterialDefinition, const mi::base::Handle<const mi::neuraylib::ICompiled_material>& CompiledMaterial)
{
	...
	FString EncodedModuleName(MaterialDefinition->get_mdl_module_name());
    const mi::IString* DecodedModuleName = MDLModule->MDLFactory->decode_name(TCHAR_TO_UTF8(*EncodedModuleName));
	FString ModuleName = UnmangleMdlPath(DecodedModuleName->get_c_str());
				...
				for (mi::Size AnnotationBlockIndex = 0, AnnotationBlockSize = AnnotationBlock->get_size(); AnnotationBlockIndex < AnnotationBlock->get_size(); AnnotationBlockIndex++)
				{
					...
					else if (AnnotationName.StartsWith(TEXT("::anno::in_group")))
					{
						...
						else if (ParameterExpression->IsA<UMaterialExpressionTextureObjectParameter>())
						{
							...
							TextureObjectParameterExpression->Group = *InGroups[0];

							if (TextureObjectParameterExpression->Texture
								&& TextureObjectParameterExpression->Texture->GetOuter()
								&& TextureObjectParameterExpression->Texture->GetOuter()->GetName() == TEXT("/MDL/Textures/DefaultColor"))
							{
