...
FMDLTranslatorResult* FMDLTemplateExporter::IsCached(const FStaticParameterSet& InStaticParamSet)
{
	...
	for (int32 Index = 0; Index < TranslatorResults->Num(); ++Index)
	{
		...
		if (TranslatorResult.StaticParamSet.bHasMaterialLayers != InStaticParamSet.bHasMaterialLayers)
		{
			continue;
		}

		if (TranslatorResult.StaticParamSet.bHasMaterialLayers && TranslatorResult.StaticParamSet.MaterialLayers.GetStaticPermutationString() != InStaticParamSet.MaterialLayers.GetStaticPermutationString())
		{
			continue;
		}
...
#undef SET_FLAG
		...
		IgnoredProperties.Add(MP_FrontMaterial); //5.0
		IgnoredProperties.Add(MP_WorldDisplacement_DEPRECATED); //5.0
		IgnoredProperties.Add(MP_TessellationMultiplier_DEPRECATED); //5.0
...
void FMDLTemplateExporter::RemoveUnusedVariables(FMDLTranslatorResult& TranslatorResult)
{
	...
	for (int32 Index = InputVariables.Num(); Index > 0; --Index)
	{
		...
		if (UsedCount < 1) // Nothing found
		{
			...
			if (IndexInArray == 0 && InputVariables.Num() > 1)
			{
