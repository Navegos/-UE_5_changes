...
FString FMDLExporterUtility::GetLegalIdentifier(const FString& InName)
{
...
TSharedPtr<IMDLExternalExporter> FMDLExporterUtility::ExternalExporter;
...
bool FMDLExporterUtility::ExportMDL(UMaterialInterface* InMaterial, const FMDLExporterSetting& InSetting, FString& MDL, FMDLTranslatorResult* OutResult, FExportTextureCallback InCallback)
{
	...
	if (ExternalExporter.IsValid() && ExternalExporter->ExportMDL(InMaterial, InSetting.Name, MDL))
	{
		return true;
	}
	else
