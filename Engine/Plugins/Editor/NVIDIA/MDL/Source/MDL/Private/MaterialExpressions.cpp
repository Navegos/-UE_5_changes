...
static void CheckedConnect(UObject* Parent, const FMaterialExpressionConnection& Connection, FExpressionInput& Input, TObjectPtr<UTexture>* OutTexturePtr)
{
...
UMaterialExpressionMakeMaterialAttributes* NewMaterialExpressionMakeMaterialAttributes(UObject* Parent,
	...
	const FMaterialExpressionConnection& SubsurfaceColor,
...
{
	check(IsScalar(Metallic) && IsScalar(Specular) && IsScalar(Roughness) && IsScalar(Opacity) && IsScalar(OpacityMask) && IsScalar(ClearCoat)
...
static T* NewMaterialExpressionTextureType(UObject* Parent, UTexture* Texture)
{
	...
	{
		Expression->Texture = LoadObject<UTexture2D>(nullptr, TEXT("/MDL/Textures/DefaultColor"), nullptr, LOAD_None, nullptr);
	}
...
UMaterialExpressionCustom* NewMaterialExpressionObjectBounds(UObject* Parent)
{
	...
	Expression->OutputType = ECustomMaterialOutputType::CMOT_Float3;
	Expression->Code = TEXT("float3(GetPrimitiveData(Parameters).ObjectBoundsX, GetPrimitiveData(Parameters).ObjectBoundsY, GetPrimitiveData(Parameters).ObjectBoundsZ);");
	return Expression;
}
...
UMaterialExpressionCustom* NewMaterialExpressionObjectPositionWS(UObject* Parent)
{
	...
	Expression->OutputType = ECustomMaterialOutputType::CMOT_Float3;	
	Expression->Code = TEXT("LWCToFloat(GetObjectWorldPosition(Parameters));");
	return Expression;
}
...
UMaterialExpressionCustom* NewMaterialExpressionObjectRadius(UObject* Parent)
{
	...
	Expression->Code = TEXT("GetPrimitiveData(Parameters).ObjectRadius");
	return Expression;
}
...
UMaterialExpressionCustom* NewMaterialExpressionLocalObjectBoundsMin(UObject* Parent)
{
	...
	Expression->OutputType = ECustomMaterialOutputType::CMOT_Float3;
	Expression->Code = TEXT("GetPrimitiveData(Parameters).LocalObjectBoundsMin;");
	return Expression;
}
...
UMaterialExpressionCustom* NewMaterialExpressionLocalObjectBoundsMax(UObject* Parent)
{
	...
	Expression->OutputType = ECustomMaterialOutputType::CMOT_Float3;
	Expression->Code = TEXT("GetPrimitiveData(Parameters).LocalObjectBoundsMax;");
	return Expression;
}
