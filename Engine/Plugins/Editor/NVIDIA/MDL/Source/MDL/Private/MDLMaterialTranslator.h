...
public:
	...
	virtual int32 NumericParameter(EMaterialParameterType ParameterType, FName ParameterName, const UE::Shader::FValue& InDefaultValue) override
	{
		...
		auto RegisterVectorName = [&](FString& Name, const FLinearColor& ReplacedValue)
		{
		...
		case EMaterialParameterType::Scalar:
		{
			...
			{
				ReplacedValue = DefaultValue.AsFloatScalar();
			}
		...
		case EMaterialParameterType::Vector:
		{
			...
			{
				ReplacedValue = DefaultValue.AsLinearColor();
			}
			
			RegisterVectorName(Name, ReplacedValue);
		...
		{
			FVector4d ReplacedValue;
			if (!GetDoubleVectorValue(MaterialInstance, ParameterName, ReplacedValue))
			{
				ReplacedValue = DefaultValue.AsVector4d();
			}

			RegisterVectorName(Name, FLinearColor(ReplacedValue));

			int32 Ret = AddInlinedCodeChunk(MCT_Float4,TEXT("%s"),*Name);
			if (FMaterialAttributeDefinitionMap::GetShaderFrequency(MaterialProperty) == SF_Pixel)
			{
				ConstantChunks.Add(Ret, FLinearColor(ReplacedValue));
			}
			return Ret;
		}
		}
	...
	*/	
	...
	virtual int32 Normalize(int32 X) override
	{
		...
		if (GetParameterUniformExpression(X))
		{
			return AddUniformExpression(new FMaterialUniformExpressionNormalize(GetParameterUniformExpression(X)), ResultType, TEXT("math::normalize(%s)"), *GetParameterCode(X));
		}
		else
		{
			return AddCodeChunk(ResultType, TEXT("math::normalize(%s)"), *GetParameterCode(X));
		}
	}
	...
	virtual int32 Step(int32 Y, int32 X) override
	{
		...
		return AddCodeChunkZeroDeriv(ResultType, TEXT("math::step(%s,%s)"), *CoerceParameter(Y, ResultType), *CoerceParameter(X, ResultType));
	}
		...
		if (IsRefractionProperty())
		{
			...
			if (CodeChunk.Type == MCT_Float)
			{
				...
				else
				{
					...
					if (CodeChunk.DefinitionFinite.StartsWith(ACode))
					{
						Fresnel = true;
					}
				}
	...
	virtual int32 StaticTerrainLayerWeight(FName LayerName,int32 Default) override
	{
		...
		else
		{
			int32 WeightmapCode;
			{
				...
				FString WeightmapName = FString::Printf(TEXT("Weightmap%s"), *LayerName.ToString());
	...
	virtual FString CoerceParameter(int32 Index, EMaterialValueType DestType) override
	{
		check(Index >= 0 && Index < CurrentScopeChunks->Num());
		const FShaderCodeChunk&	CodeChunk = (*CurrentScopeChunks)[Index];
		EMaterialValueType SourceType = CodeChunk.Type;
		...
		else if (IsFloatNumericType(SourceType) && IsFloatNumericType(DestType))
		{
			...
			if ((SourceType & DestType) && (SourceType & MCT_Float))
			{
				switch (DestType)
				{
				case MCT_Float1:
			...
			{
				uint32 NumComponents = FMath::Min(NumSourceComponents, NumDestComponents);
				if (NumComponents < NumSourceComponents)
				{
					switch (DestType)
					{
					case MCT_Float1:
						return FString::Printf(TEXT("%s.x"), *GetParameterCode(Index));
					case MCT_Float2:
						return FString::Printf(TEXT("float2(%s.x,%s.y)"), *GetParameterCode(Index), *GetParameterCode(Index));
					case MCT_Float3:
						return FString::Printf(TEXT("float3(%s.x,%s.y,%s.z)"), *GetParameterCode(Index), *GetParameterCode(Index), *GetParameterCode(Index));
					case MCT_Float4:
						return FString::Printf(TEXT("float4(%s.x,%s.y,%s.z,%s.w)"), *GetParameterCode(Index), *GetParameterCode(Index), *GetParameterCode(Index), *GetParameterCode(Index));
					}
				}
			}
		}

		Errorf(TEXT("Coercion failed: %s: %s -> %s"), *CodeChunk.DefinitionFinite, DescribeType(SourceType), DescribeType(DestType));
		return TEXT("");
	}
	...
	virtual int32 TextureCoordinate(uint32 CoordinateIndex, bool UnMirrorU, bool UnMirrorV) override
	{
		...
		const uint32 MaxNumCoordinates = ((FeatureLevel != ERHIFeatureLevel::ES2_REMOVED) || Material->IsUIMaterial() || Material->IsUsedWithLandscape()) ? 8 : 3;
		...
		) override
	{
		...
		if (SamplerSource == SSM_FromTextureAsset)
		{
			...
			if (UniformExpression && UniformExpression->GetTextureUniformExpression())
			{
				...
				{
					const UObject* TextureObj = Material->GetReferencedTextures()[TextureUniformExpression->GetTextureIndex()];
		...
		FString UVs = CoerceUVParameter(CoordinateIndex, UVsType);
	...
	virtual int32 Texture(UTexture* InTexture,int32& TextureReferenceIndex, EMaterialSamplerType SamplerType, ESamplerSourceMode SamplerSource=SSM_FromTextureAsset, ETextureMipValueMode MipValueMode = TMVM_None) override
	{
		...
		if(ShaderType & MCT_Texture)
		{
			...
			FCString::Sprintf(FormattedCode, TEXT("\"./%s/%s%s.%s\",%s"),
				*TexturePath,
				*(*CurrentTextureCache)[InTexture], 
				bVirtual ? TEXT(".<UDIM>") : TEXT(""),
				*FMDLExporterUtility::GetFileExtension(InTexture, DDSTexture),
	...
	bool GetDoubleVectorValue(UMaterialInstance* Instance, FName ParameterName, FVector4d& Return)
	{
		if (Instance)
		{
			for (auto DoubleVectorParameter : Instance->DoubleVectorParameterValues)
			{
				if (DoubleVectorParameter.ParameterInfo.Name.Compare(ParameterName) == 0)
				{
					Return = DoubleVectorParameter.ParameterValue;
					return true;
				}
			}
			...
			if (Instance->Parent->IsA<UMaterialInstance>())
			{
				return GetDoubleVectorValue(Cast<UMaterialInstance>(Instance->Parent), ParameterName, Return);
			}
		}
		...
		if (DefinedName == nullptr)
		{
			...
			else
			{
				...
				TranslatedCodeParameters.Add(FString::Printf(TEXT("%s\t%s%s %s = %s(\"./%s/%s%s.%s\",%s)%s"),
					...
					*(*CurrentTextureCache)[ReplacedTexture],
					bVirtual ? TEXT(".<UDIM>") : TEXT(""),
					*FMDLExporterUtility::GetFileExtension(ReplacedTexture, DDSTexture),
	...
	virtual int32 AccessUniformExpression(int32 Index) override
	{
		...
		else if (CodeChunk.Type & MCT_Texture)
		{
			...
			else
			{
				...
				FCString::Sprintf(FormattedCode, TEXT("%s(%s)"), BaseName, *CodeChunk.DefinitionFinite);
			}
		}
		else
		{
	...
	virtual int32 CustomOutput(class UMaterialExpressionCustomOutput* Custom, int32 OutputIndex, int32 OutputCode) override
	{
		...
		{
			GetFixedParameterCode(OutputCode, *CurrentScopeChunks, Definitions, Body, ECompiledPartialDerivativeVariation::CompiledPDV_FiniteDifferences);
		}
	...
	virtual int32 VertexColor() override
	{
		...
		bVertexColorUsed = true;
		RegisterIntParameter(TEXT("VertexColorCoordinateIndex"));
		return AddInlinedCodeChunk(MCT_Float4, TEXT("::vertex_color_from_coordinate(VertexColorCoordinateIndex)"));
	}
	...
	int32 FresnelFunction(class UMaterialExpressionMaterialFunctionCall* FresnelFunc, FMaterialCompiler* Compiler, int32 OutputIndex)
	{
		...
		return AddCodeChunk(MCT_Float,TEXT("::fresnel_function(%s, %s, %s, %s, %s, %s, %s, %s)"),  
			*CoerceParameter(NormalArg,MCT_Float3),
			*CoerceParameter(CameraArg,MCT_Float3),
			*GetParameterCode(InvertFresnelArg), 
			*GetParameterCode(PowerArg), 
			*GetParameterCode(UseCheapContrastArg),
			*GetParameterCode(CheapContrastDarkArg), 
			*GetParameterCode(CheapContrastBrightArg),
			*GetParameterCode(ClampFresnelDotProductArg)
			);
	}
	...
	virtual int32 CallExpression(FMaterialExpressionKey ExpressionKey, FMaterialCompiler* Compiler) override
	{
		...
		else
		{
			...
			else
			{
				...
				{
					if ((*CurrentScopeChunks)[Result].DefinitionFinite.StartsWith(TEXT("::scene_depth()")))
					{
	...
	void ExtractConstantProperties(const TArray<EMaterialProperty>& IgnoredProperties, TMap<EMaterialProperty, FLinearColor>& OutConstantProperties)
	{
		for (int32 PropertyIndex = 0; PropertyIndex < MP_MAX; ++PropertyIndex)
		{
			...
			for (auto Chunk : ResultChunk)
			{
				...
				if (DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunks[PropertyIndex].Contains(Chunk.SymbolName))
				{
					ConstantInput = false;
					break;
				}
	...
	virtual int32 AddCodeChunkInner(uint64 Hash, const TCHAR* FormattedCode, EMaterialValueType Type, EDerivativeStatus DerivativeStatus, bool bInlined) override
	{
		const int32 NewIndex = CurrentScopeChunks->Num();
		int32 CodeIndex = FHLSLMaterialTranslator::AddCodeChunkInner(Hash, FormattedCode, Type, DerivativeStatus, bInlined);	
	...
	void GetSharedInputsMaterialCode(const TArray<EMaterialProperty>& IgnoredProperties, TArray<FString>& ParametersCode, FString& NormalCode, FString& NormalAssignment, FString& VertexMembersInitializationEpilog, FString& PixelMembersInitializationEpilog)
	{
		...
		bool bSceneColorTranslucent = MaterialShadingModels.GetFirstShadingModel() == MSM_Unlit && IsTranslucentBlendMode(Material->GetBlendMode()) && bSceneColorUsed && DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunks[MP_Opacity] == TEXT("1.0");
		bool bSceneDepthTranslucent = MaterialShadingModels.GetFirstShadingModel() == MSM_Unlit && IsTranslucentBlendMode(Material->GetBlendMode()) && bSceneDepthOpacity;
		{
			...
			for (int32 PropertyIndex = 0; PropertyIndex < MP_MAX; ++PropertyIndex)
			{
				...
				{
					NormalInitializerValue = FString::Printf(TEXT("\t%s %s = %s;\r\n"), HLSLTypeString(Type), *PropertyName, *DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunks[Property]);
				}
				else
				{
					if (DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunkDefinitions[Property].Len() > 0)
					{
					...
					{
						PixelInputInitializerValues += FString::Printf(TEXT("\t%s %s = (%s - %s) < 0.0f ? 0.0f : 1.0f;\r\n"), HLSLTypeString(Type), *PropertyName, *DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunks[Property], FL(Material->GetOpacityMaskClipValue()));
					}
					else if (Property == MP_Refraction)
					{
						...
						{
							PixelInputInitializerValues += FString::Printf(TEXT("\t%s %s = float2(%s);\r\n"), HLSLTypeString(Type), *PropertyName, FL(DefaultRefraction));
						}
						else
						{
					...
					{
						PixelInputInitializerValues += FString::Printf(TEXT("\t%s %s = 1.0f - %s.x;\r\n"), HLSLTypeString(Type), *PropertyName, *DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunks[MP_EmissiveColor]);
					}
					...
					if (Property == MP_Opacity)
					{
						if (IsTranslucentBlendMode(Material->GetBlendMode()))
						{
							if (Material->GetCastDynamicShadowAsMasked())
							{
								...
								{
									PixelInputInitializerValues += FString::Printf(TEXT("\tfloat OpacityMask_mdl = (math::saturate(1.0f - %s.x) - %s) < 0.0f ? 0.0f : 1.0f;\r\n"), *DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunks[MP_EmissiveColor], FL(Material->GetOpacityMaskClipValue()));
								}
								else
								{
									PixelInputInitializerValues += FString::Printf(TEXT("\tfloat OpacityMask_mdl = (math::saturate(%s) - %s) < 0.0f ? 0.0f : 1.0f;\r\n"), *DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunks[Property], FL(Material->GetOpacityMaskClipValue()));
								}
							}
			...
			if (LastProperty != -1)
			{
				PixelMembersInitializationEpilog += DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunkDefinitions[LastProperty] + LINE_TERMINATOR;;
			}
			...
			if (LastProperty != -1)
			{
				VertexMembersInitializationEpilog += DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunkDefinitions[LastProperty] + LINE_TERMINATOR;;
			}

			VertexMembersInitializationEpilog += VertexInputInitializerValues;

			NormalCode = DerivativeVariations[CompiledPDV_FiniteDifferences].TranslatedCodeChunkDefinitions[MP_Normal];
	...
	void CheckUniqueTextureName(UTexture* InTexture)
	{
		if (CurrentTextureCache->Find(InTexture) == nullptr)
		{
			...
			if (CurrentTextureCache->FindKey(InTexture->GetName()))
			{
				// name is not unique
				ID = TEXT("_") + FString::FromInt(FMDLExporterUtility::GlobalTextureID++);
			}

			CurrentTextureCache->Add(InTexture, InTexture->GetName() + ID);
		}
	}
