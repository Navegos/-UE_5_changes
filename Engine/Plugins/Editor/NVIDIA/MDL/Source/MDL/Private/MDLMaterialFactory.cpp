...
UObject* UMDLMaterialFactory::FactoryCreateFile(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, const FString& Filename, const TCHAR* Parms, FFeedbackContext* Warn, bool& bOutOperationCanceled)
{
	...
	if (ModulePath.StartsWith(MDLModule->MDLUserPath + TEXT("/nvidia/vMaterials")))
	{
		...
		PackageName = FMDLImporterUtility::GetProjectMaterialPath() / PackageName / InName.ToString();
	...
	TArray<UObject*> ResultAssets;
	{
		...
		for (mi::Size i = 0; i < ResultCount; i++)
		{
			...
			mi::base::Handle<const mi::neuraylib::IMaterial_definition> MaterialDefinition = MDLModule->WrapperMaterialPath.IsEmpty()
				? mi::base::make_handle(MDLModule->Transaction->access<mi::neuraylib::IMaterial_definition>(FileModule->get_material(i)))
			...
			mi::Sint32 ErrorCode;
			const mi::base::Handle<const mi::neuraylib::IMaterial_instance> MaterialInstance(MaterialDefinition->create_material_instance(nullptr, &ErrorCode));
			if (ErrorCode < 0)
			{
			...
			// Compile the Material (according to settings either Instance- or Class-Compilation)
			...
			mi::base::Handle<mi::neuraylib::IMdl_execution_context> Context(MDLModule->MDLFactory->create_execution_context());
			...
			if (ErrorCode < 0)
			{
				Warn->Logf(ELogVerbosity::Error, TEXT("Failed to compile MDL material '%s', error code is %d"), *MaterialName, ErrorCode);
				continue;
			}
			...
			if (!bImportSuccess)
			{
				auto InvalidFunctions = MaterialImporter->GetLastInvalidFunctionCalls();
			...
			Material->MaterialAttributes.Connect(0,
				NewMaterialExpressionFunctionCall(Material,
					::LoadFunction(FMDLImporterUtility::GetProjectFunctionPath(), TEXT("mdlimporter_set_clip_mask")), { Material->MaterialAttributes.Expression }));
...
MaterialData UMDLMaterialFactory::GetMaterialData(const FString& MaterialName, mi::Size StartIndex)
{
	...
	// Access material
	const mi::base::Handle<const mi::neuraylib::IMaterial_definition> MaterialDefinition(MDLModule->Transaction->access<mi::neuraylib::IMaterial_definition>(TCHAR_TO_ANSI(*MaterialName)));
	mi::Size ParameterCount = MaterialDefinition->get_parameter_count();

	// Get a MaterialInstance from the MaterialDefinition
	mi::Sint32 ErrorCode;
	mi::base::Handle<mi::neuraylib::IMaterial_instance> MaterialInstance(MaterialDefinition->create_material_instance(nullptr, &ErrorCode));
	verify(ErrorCode == 0);
...
{
	mi::base::Handle<const mi::neuraylib::IMaterial_definition> MaterialDefinition = mi::base::make_handle(MDLModule->Transaction->access<mi::neuraylib::IMaterial_definition>(TCHAR_TO_ANSI(*MaterialName)));
...
const mi::base::Handle<const mi::neuraylib::IModule> UMDLMaterialFactory::PrepareModule(FString& ModulePath, const FString& ModuleName, FFeedbackContext* Warn)
{
	...
	int32 ErrorCode = MDLModule->LoadModule(ModuleName);
	switch (ErrorCode)
	{
		case 1:
			...
			// fall-through !!
		case 0:
			Module = MDLModule->GetLoadedModule(ModuleName);
...
#if WITH_MDL_SDK
mi::base::Handle<const mi::neuraylib::IMaterial_definition> UMDLMaterialFactory::WrapMaterial(const ModuleData& WrappeeModuleData, const FString& Material, FFeedbackContext* Warn)
{
	...
	case 0:
		...
		MaterialDefinition = mi::base::make_handle(MDLModule->Transaction->access<mi::neuraylib::IMaterial_definition>(TCHAR_TO_ANSI(*WrappedMaterialModule)));
		break;
	default:
