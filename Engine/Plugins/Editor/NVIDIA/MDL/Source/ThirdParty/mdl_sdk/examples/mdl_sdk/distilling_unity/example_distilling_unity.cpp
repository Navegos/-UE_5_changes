...
public:
    ...
    // Only get the color map, do not attempt to create it
    mi::neuraylib::ICanvas * get_base_color_map() const
    {
        if (!m_base_color_map.is_valid_interface())
        {
            return NULL;
    ...
    // Only get the color map, do not attempt to create it
    mi::neuraylib::ICanvas * get_mask_map() const
    {
        if (!m_mask_map.is_valid_interface())
        {
            return NULL;
    ...
    {
        if (param_name == "base_color" || param_name == "transparency")
        {
            return get_base_color_map(image_api, resolution);
        }
        if (param_name == "metallic" || param_name == "roughness")
        {
...
private:
    ...
    ) const
    {
        ...
        for (auto & p : plist)
        {
            ...
            if (is_uniform(m_native_code.get(), function_description.function_index))
            {
                // 1 pixel canvas
                samples = 1;
                resolution = 1;
    ...
    ) const
    {
        ...
        CUcontext cuda_context = init_cuda(m_cuda_device);
        {
            ...
            for(auto & p : plist)
            {
                ...
                mi::Uint32 resolution(baking_resolution);
                // Determine if need to bake texture or constant
                if (is_uniform(m_gpu_code.get(), function_description.function_index))
                {
                    // 1 pixel canvas
                    samples = 1;
                    resolution = 1;
    ...
    mi::neuraylib::IMdl_impexp_api* impexp_api)
{
        ...
        it != material.end(); ++it)
    {
        ...
        if (param_name == "transparency" && material.get_base_color_map())
        {
            continue;
        }
        if (param_name == "roughness" && material.get_mask_map())
        {
            continue;
        }

        log_message << "Parameter: '" << param_name << "': ";
        if(param.bake_path.empty())
        {
            log_message << " no matching bake path found in target material.";

            if(param.value)
                log_message << "--> value set to ";
            if(param.texture)
                log_message << "--> calculated ";
        }
        else
        {
            log_message << "path '" << param.bake_path << "' baked to ";
        }
    ...
    , std::vector<std::string> & material_names)
{
    ...
    for (auto module_name : module_names)
    {
        ...
        for (mi::Size i = 0; i < material_count; i++)
        {
            ...
            mi::base::Handle<const mi::neuraylib::IMaterial_definition> material(
                transaction->access<mi::neuraylib::IMaterial_definition>(mname.c_str()));
